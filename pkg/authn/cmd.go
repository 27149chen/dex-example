...
func RootCmd() *cobra.Command {
	...
	c := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "https":
				log.Printf("listening on %s", listen)
				return http.ListenAndServeTLS("443", tlsCert, tlsKey, nil)
			default:
				return fmt.Errorf("listen address %q is not using http or https", listen)
			}
		},
	}
	c.Flags().StringVar(&a.clientID, "client-id", "gloo", "OAuth2 client ID of this application.")
	c.Flags().StringVar(&a.clientSecret, "client-secret", "secretvalue", "OAuth2 client secret of this application.")
	c.Flags().StringVar(&a.redirectURI, "redirect-uri", "http://1.117.135.124/callback", "Callback URL for OAuth2 responses.")
	c.Flags().StringVar(&issuerURL, "issuer", "http://dex.test.8slan.com/dex", "URL of the OpenID Connect issuer.")
	c.Flags().StringVar(&listen, "listen", "http://1.117.135.124", "HTTP(S) address to listen at.")
